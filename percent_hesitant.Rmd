...
GET("https://data.cdc.gov/resource/hkhc-f7hg.csv")
```
...
broom::tidy(fit2a)
```
# Models for hesitancy and SES
```{r}
#outcome : binary (hesitant vs. non-hesitant) --> logistic regression 
#predictor: social vulnerability index (continuous) 
#will do a model adjusting for %race of a county & state 
#can also make a binary variable (white & non-white)

```

## Map stuff for hestiancy 
```{r}
glimpse(vax)
install.packages("leaflet")
library(leaflet)
?spplot
leaflet(vax$county_boundary, "estimated_hesitant")
```
